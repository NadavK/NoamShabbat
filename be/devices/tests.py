...
class DeviceTests(APITestCase):
    ...
    def test_health(self):
        ...
        self.assertLess(abs(device.created_time - timezone.now()), timedelta(seconds=1))
        ...
        self.assertEqual(Device.objects.count(), 1)
        device = Device.objects.get(sn='1')
        self.assertEqual(device.pk, 1)
        self.assertEqual(device.name, 'חדש 1')
        self.assertEqual(device.ip, '1.1.1.1')
        self.assertEqual(device.created_time, created_time)
        self.assertLess(abs(device.last_health_time - timezone.now()), timedelta(seconds=1))
        ...
        self.assertEqual(device.pk, 1)
        self.assertEqual(device.name, 'חדש 1')
        self.assertEqual(device.ip, '1.1.1.0')
        self.assertLess(abs(device.last_health_time - timezone.now()), timedelta(seconds=1))

        response = self.client.post(url, {'sn': '2', 'ip': '2.2.2.2'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Device.objects.count(), 2)
        device = Device.objects.get(sn='2')
        self.assertEqual(device.pk, 2)
        self.assertEqual(device.name, 'חדש 2')
        self.assertEqual(device.ip, '2.2.2.2')
        self.assertLess(abs(device.last_health_time - timezone.now()), timedelta(seconds=1))
